...
PKPs are public/private key pairs generated by the Lit network in a process called Distributed Key Generation. Each node custodies a *share* of the underlying private key, meaning the key **never exists in its entirety**. Currently, users can mint a PKP in the form of an ERC-721 NFT. Only the owner of the NFT has the ability to query the Lit network to combine the underlying private key shares, their wallet signature standing as the method of authorization for the Lit nodes.

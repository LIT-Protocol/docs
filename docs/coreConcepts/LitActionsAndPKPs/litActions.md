...
Signing through Lit Actions opens up the possibilities of verifying information from external sources, such as a Weather API. Within the Lit ecosystem, this is considered a proof. The proof sends back information from a source such as endpoint or function, which you can then harness within your own application. 

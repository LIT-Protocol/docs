...
PKPs are the public/private key pairs generated by the Lit network and are minted in the form of an ERC-721 NFT. Only those with authorized access to sign with the PKP (via an [auth sig](/SDK/Explanation/WalletSigs/sessionSigs)) can combine the underlying key shares.
